...
to decide-contribution
  ...
  while [current-contribution <= my-income + .0001 ] ;floating-point manissa
  [
    ...
    set current-contribution current-contribution + .05
  ]
  ...
  if (expulsion and my-IO-contribution = 0) [leave-institution]
end
...
to-report my-expected-utility-single [requested contribution]
  let funds expected-IO-investible + contribution
  ...
  let defection-percent (requested - contribution) / requested
  let w 1 - (defection-percent * my-shadow-future) ;an estimate of future harm that might come from short-term gain
  let EV-investment (expected-payout-single-issue - contribution) * w
  let remaining my-income - contribution ;assume that any leftover funds have a utility of 1:1
  report  EV-investment + remaining
end

;turtle or observer context
to-report expected-IO-investible
  report expected-IO-income-others - expected-IO-costs + IO-wealth - IO-reserve
end
...
to allocate-IO-funds
  ...
  let investible IO-wealth - IO-reserve
  ifelse (investible >= 0)
  [
...
to update-membership
  ...
  [
    ...
    if not satisfied-with-IO [leave-institution]
  ]
...
to-report issue-benefit[issue]
  if (issue = "security")[report multipliers]
  if (issue = "trade" or issue = "culture")[report multipliers ]
  if (issue = "human-rights" or issue = "environment")[report multipliers ]
end
